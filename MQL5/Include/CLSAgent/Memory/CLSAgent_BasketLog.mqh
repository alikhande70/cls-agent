...
//|   only when a direction's order count actually changed since the        |
//|   last bar - opened, grew (burst add), shrank (partial exit/manual       |
//|   close), or closed flat - not unconditionally every bar.                 |
//+------------------------------------------------------------------+
...
void CLS_BasketLog_CheckDirection(const string symbol, const ENUM_CLS_DIRECTION direction)
{
   ...
   if(count == g_BasketLogLastCount[idx])
      return;
